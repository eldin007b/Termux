...
const footer = blessed.box({
    ...
    tags: true, align: 'center',
    content: '→ Desno/Enter: POKRENI | ← Lijevo: TERMINAL'
});
...
function executeAction(index) {
    switch(index) {
        ...
        case 4: triggerCommand('TERMINAL'); break; // Samo izlaz iz petlje
        case 5: triggerCommand('EXIT'); break;     // Totalno gašenje
    }
}
...
list.on('keypress', (ch, key) => {
    ...
    if (key.name === 'left') triggerCommand('TERMINAL');
});

screen.key(['q', 'C-c'], () => triggerCommand('TERMINAL'));
screen.key(['1','2','3','4','5','0'], (ch) => {
    const map = {'1':0, '2':1, '3':2, '4':3, '5':4, '0':5};
    if(map[ch] !== undefined) {
        list.select(map[ch]);
